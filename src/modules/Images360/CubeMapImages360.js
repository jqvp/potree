...
let sgHigh = new THREE.BoxGeometry(128,128,128);
...
class CubeMapImage360{

	constructor(files, time, longitude, latitude, altitude, course, pitch, roll){
		this.files = files; // px nx py ny pz nz
		this.time = time;
...
export class CubeMapImages360 extends EventDispatcher{
	...
	constructor(viewer){
		...
		this.cube = new THREE.Mesh(sgHigh, sm);
	...
	set visible(visible){
		...
		this.cube.visible = visible && (this.focusedImage != null);
	...
	focus(image360){
		...
		this.load(image360).then( () => {
			...
			this.cube.material.envMap = image360.texture;
		...
		this.cube.position.set(...image360.position);
	...
	load(image360){
		...
		return new Promise(resolve => {
			let texture = new THREE.CubeTextureLoader().load(...image360.files, resolve);
...
export class CubeMapImages360Loader{
	/**
	 * 
	 * @param {String} url 
	 * @param {Viewer} viewer 
	 * @param {{*}} params 
	 * @returns {CubeMapImages360} An object with all the 360 images specified in the content 
	 * of the URL
	 */
	static async load(url, viewer, params = {}){
		...
		let images360 = new CubeMapImages360(viewer);
		...
		for(let line of coordinateLines){
			...
			let [filenames, time, long, lat, alt, course, pitch, roll] = tokens;
			...
			filenames = filenames.split(/,/).map(x => x.replace(/"/g, ""));
			let files = filenames.split(/,/).map(x => `${url}/${x.replace(/"/g, "")}`);

			let image360 = new CubeMapImage360(files, time, long, lat, alt, course, pitch, roll);
		...
		CubeMapImages360Loader.createSceneNodes(images360, params.transform);
